...
/*
    ...
    @arg nspeaker is number of speakers
    @arg rolloff is gain reduction factor in db
    @rs is width of source
*/
...
with {
    ...
    distance(x,y,z, xspeaker, yspeaker, zspeaker, rs) = pow(x-xspeaker, 2) 
                                                        ...
                                                        : +(pow(rs, 2))
                                                        : sqrt;
    dia(x,y,z, xsp, ysp, zsp, rs) = pow(distance(x, y, z, xsp, ysp, zsp, rs), (0.5*a(rolloff))); 
    ...
    with {
        ...
        with {
            ...
            y = ba.take(2, src);
            z = ba.take(3, src);
            proc_dia(n) = dia(x, y, z, xsp, ysp, zsp, rs) 
            with {
...
graphics(n, sig) = sig <: attach(_, abs : hbargraph("Amp_%n", 0, 1));
...
process = dbap(NSRC, NSPEAKERS, rolloff, rs,src*gain ) : par(n, NSPEAKERS, graphics(n));
